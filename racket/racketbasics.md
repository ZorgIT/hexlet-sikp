...
основные функуции format и printf (формат вычисляется в строку, а printf выводит сразу на экран)
...
~a принимает одно или несколько значений, которые будет преобразованы в строки и объединены в результирующую строку. 
...
устойчивость к имзенению это свойство конкретного строкового значения . проверить можно коммандой immutable?
...
Обратное преобразование не требует отдельной функции, вместо неё используется функция string-copy, которая создаёт изменяемую копию любой строки.
...
Рассмотрим две функции, которые модифицируют содержимое изменяемой строки "по месту", не создавая изменённой копии. Это будут функции string-set! и string-copy!
...
define s (string-copy ".Tod")) - копирование строки в саму сеяб
